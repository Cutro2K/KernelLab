...
export function worstFitCon(processes: Process[], memoryState: MemoryBlock[], _params: SimulationConfig): SimulationStep[] {
	...
	while (pending.length > 0 || hasRunningProcesses(state)) {
		...
		if (arrived.length > 0) {
			...
			for (let pIndex = 0; pIndex < pending.length; pIndex += 1) {
				...
				let worstBlockIndexForProcess = -1;
				let worstBlockSizeForProcess = -1;

				for (let bIndex = 0; bIndex < state.length; bIndex += 1) {
					const block = state[bIndex];
					if (!block.isFree || block.size < process.size) continue;

					if (block.size > worstBlockSizeForProcess) {
						worstBlockSizeForProcess = block.size;
						worstBlockIndexForProcess = bIndex;
					}
				}

				if (worstBlockIndexForProcess !== -1) {
					selectedProcess = process;
					selectedPendingIndex = pIndex;
					selectedBlockIndex = worstBlockIndexForProcess;
					break;
				}
			}
