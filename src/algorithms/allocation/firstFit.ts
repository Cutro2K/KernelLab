...
export function firstFitCon(processes: Process[], memoryState: MemoryBlock[], _params: SimulationConfig): SimulationStep[] {
    ...
    while (pending.length > 0 || hasRunningProcesses(state)) {
        ...
        if (arrived.length > 0) {
            ...
            for (let pIndex = 0; pIndex < pending.length; pIndex += 1) {
                ...
                const firstBlockIndex = state.findIndex((block) => block.isFree && block.size >= process.size);
                if (firstBlockIndex !== -1) {
                    selectedProcess = process;
                    selectedPendingIndex = pIndex;
                    selectedBlockIndex = firstBlockIndex;
                    break;
                }
            }
        ...
        if (allocatedProcessName) {
            descriptionParts.push(`Asignacion (First Fit): ${allocatedProcessName} en el primer bloque compatible.`);
        }
